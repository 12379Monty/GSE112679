...
```{r  readTUTI, eval=T, cache=T, cache.vars=c('TUTI_featureCount', 'genes_annot'),message=F}
 ...
 data.frame(Symbol=sapply(strsplit(rownames(TUTI_featureCount), split='~'), function(x) rev(x)[1]),
             ...
             Start=sapply(strsplit(rownames(TUTI_featureCount), split='~'), '[', 3),
             Stop=sapply(strsplit(rownames(TUTI_featureCount), split='~'), '[', 4),
...
THIS.DONT.WORK <- function() {
 ...
 save(list='TUTI_featureCount', file=file.path(data_DIR, 'TUTI_featureCount.rda'))
 save(list='Train_featureCount', file=file.path(data_DIR, 'Train_featureCount.rda'))
 save(list='Val1_featureCount', file=file.path(data_DIR, 'Val1_featureCount.rda'))
 save(list='Val2_featureCount', file=file.path(data_DIR, 'Val2_featureCount.rda'))
